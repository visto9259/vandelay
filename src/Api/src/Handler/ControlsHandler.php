...
readonly class ControlsHandler extends AbstractHandler
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($request->getMethod() === 'GET') {
            ...
            $data           = $this->chorusService->getControlService()->getControls($appId, $installationId, $queryParams);
            forEach ($data as $index => $item) {
                $details = $this->chorusService->getControlService()->getControlDetails($appId, $installationId, $item['id']);
